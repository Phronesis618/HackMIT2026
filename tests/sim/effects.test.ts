...
import { ATTUNEMENT_EFFECT_IDS, ATTUNEMENT_EFFECT_INFO, ENEMY_INFO, type AttunementEffectId } from '../../src/shared/registry';
...
describe('attunement effects in the simulation', () => {
  ...
  it('core nodes: Second Wind shortens the dash cooldown 25%; Salvager pays 1 more per clear', () => {
    ...
    // The one husk also paid its registry shards on defeat (Agent A: hostiles pay ENEMY_INFO.shards).
    const killPay = ENEMY_INFO.husk.shards;
    expect(me(plain).resources! - before.plain).toBe(ROOM_CLEAR_REWARD + killPay);
    expect(me(boosted).resources! - before.boosted).toBe(ROOM_CLEAR_REWARD + 1 + killPay);
  });
});

...
export function AbilityBar({ model, actions }: { model: UiModel; actions: UiActions }) {
  ...
  return (
    <div className="abilitybar" style={{ ['--class-color' as string]: theme.primary, ['--class-secondary' as string]: theme.secondary }}>
      {hovered && (
      ...
      <div className="abilitybar__slots">
        {slots.map((slot) => {
          ...
          return (
            <div
              key={slot.id}
              className={`abilityslot abilityslot--${slot.state} ${isUlt ? 'abilityslot--ult' : ''}`}
            ...
            >
              <div className="abilityslot__frame">
                <AbilityIcon icon={detail.icon} color={slot.state === 'locked' || slot.state === 'unavailable' ? '#6b7690' : isUlt ? theme.secondary : theme.primary} />
                {sweep > 0 && <div className="abilityslot__sweep" style={{ height: `${sweep * 100}%` }} />}
                {sweep > 0 && <span className="abilityslot__timer">{(slot.cooldownMs / 1000).toFixed(1)}</span>}
                {isUlt && slot.state !== 'ready' && <div className="abilityslot__charge" style={{ height: `${100 - chargePct}%` }} />}
                {isUlt && slot.state !== 'ready' && <span className="abilityslot__timer">{chargePct}%</span>}
                {slot.state === 'locked' && <span className="abilityslot__lock">🔒</span>}
                {slot.state === 'ready' && isUlt && <span className="abilityslot__readyglow" />}
              </div>
              <span className="abilityslot__key">{detail.key}</span>
              <span className="abilityslot__name">{detail.name}</span>
              {slot.state === 'locked' && canUnlock && actions.unlockAbility && (
                <button type="button" className="btn btn--primary abilityslot__unlock" onClick={() => actions.unlockAbility?.()}>
                  Unlock · {ABILITY_UNLOCK_COST}
                </button>
              )}
            </div>
          );

...
export const LAW_INFO: Record<WorldLawId, {
   ...
   * offered to the model (`lawsAndLookRegistry`) and the UI marks them "not in force", so no
   * player is shown a law that does nothing. tests/sim/law-honesty.test.ts keeps this in step
  ...
  tidal_drag: { group: 'movement', budget: 0, implemented: true, summary: 'Crew and enemies walk about 17% slower; dash cooldown drops by a third. Dash more, walk less.' },
  ...
  glass_lattice: { group: 'combat', budget: 2, implemented: true, summary: 'Operatives have 48 Integrity instead of 100 and deal 1.8x damage.' },
  ...
  first_light: { group: 'combat', budget: -1, implemented: true, summary: 'The first hit on an untouched enemy deals 2.5x damage.' },
  few_and_terrible: { group: 'enemies', budget: 1, implemented: true, summary: 'Half as many enemies, each with 2.2x health and 28% more damage.' },
  the_many: { group: 'enemies', budget: 1, implemented: true, summary: '1.8x as many enemies, each at 55% health and 78% damage. Attacks that hit several at once pay off.' },
  ...
  unstable_matter: { group: 'enemies', budget: 1, implemented: false, summary: 'Enemies burst when they fall: 70 px radius, 12 damage to crew, 20 to other enemies. Bursts chain through packs.' },

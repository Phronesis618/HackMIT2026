...
## 9 · Simulation and sessions as implemented (F2)

Files: `src/sim/floors.ts` (run state, rules, every tuning number in `FLOOR_TUNING`), `src/sim/floorProvider.ts` (`createRoomProvider(world)` = the shared `createWorldFloorRuntime`; the sim and `GameController` both use it), surgical call sites in `src/sim/simulation.ts`. Legacy worlds take none of these paths.

- **Start.** `sim.enterRoom(0)` from HQ on a world with `floors` starts the run in `entranceRef()` (emits `biome_entered` tier 0, then `room_entered` with `biomeId`/`floorRoomId`/`kind`). Sessions call exactly what they called before.
- **Traversal is inside the sim.** A living operative stepping on a door tile of a cleared room emits `exit_reached` (with `toRoomId`) and the whole crew moves on the same tick, arriving on the twin door's `entry` (others fan out inward; door tiles are never a landing spot). Sessions ignore `exit_reached` events that carry `toRoomId`. Per-room state (enemies, terrain damage, relic/remains nodes) is kept by `biomeId:roomId` for the current biome and dropped when the crew moves a biome deeper.
- **Door locks.** `combat`, `elite` and `exit` rooms seal until cleared: door tiles are solid in the collision grid (`buildSolidGrid(room, brokenWalls, sealedDoors)`), `snapshot.floor.doorsLocked` is true. Rooms without enemies are cleared on entry and pay nothing. Pack members never spawn in a pocket cut off from their encounter tile (a sealed room must always be clearable).
- **Kinds.** `rest`: first touch of the focus while someone is hurt heals every living operative 40 % max HP, **once per room per run** — not once per visit. Leaving the room and coming back does not refill it (`FloorsRun.usedFeatures`), and the room description ("Stopping here mends the crew once, and not again.") and the full-map legend ("Rest site · mends the crew once") both say so. `treasure`: first touch pays 4x the clear reward to everyone, reported as `room_cleared`. Both show `cleared: true` on the map once used. `lore`: the runtime's relic on the focus, read with the normal F-hold. `elite`: clear pays 2x. `exit`: clear pays 3x; the `gatekeeper` encounter spawns as a guardian with 40 % HP held to boss phase 1. Tier-4 exit: `isFinal`, three-phase Custodian, relay ritual, `anchor_planted`, `run_ended: anchored` (all PR #16 code).
- **Scaling.** Enemy HP and damage x `1 + 0.18 * tier`.
- **Biome choice.** After a non-final exit is cleared its focus is the choice site: F within 44 px opens `snapshot.floor.biomeChoice` (`biome_choice_offered`). `sim.chooseBiome(playerId, biomeId)` records a vote; the host's vote (`sim.setHostPlayerId`, solo = the only operative) sets `chosenBiomeId`, and the next step moves the crew to the chosen biome's `r00` (`biome_entered`, `room_entered`). Wire: client message `choose_biome`, `GameSession.chooseBiome`. Until the panel exists, keys 1 / 2 pick in `GameController`.
- **Snapshot.** `floor = { biomeId, roomId, tier, path, map, doorsLocked, biomeChoice }`; `roomId` is the RoomSpec id, `roomIndex` the floor-plan index. `map` lists visited rooms plus unvisited neighbours; a neighbour's kind is hidden only while it could be a fight (`combat`/`elite` -> `null`), special rooms show from next door. No room spec ever rides in a snapshot.

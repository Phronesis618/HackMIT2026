...
- **Kinds.** `rest`: first touch of the focus while someone is hurt heals every living operative 40 % max HP, once per room per run. `treasure`: first touch pays 4x the clear reward to everyone, reported as `room_cleared`. Both show `cleared: true` on the map once used. `lore`: the runtime's relic on the focus, read with the normal F-hold. `elite`: clear pays 2x. `exit`: clear pays 3x; the `gatekeeper` encounter spawns as a guardian with 40 % HP held to boss phase 1. Tier-4 exit: `isFinal`, three-phase Custodian, relay ritual, `anchor_planted`, `run_ended: anchored` (all PR #16 code).

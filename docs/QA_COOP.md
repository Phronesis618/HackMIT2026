...
| 4c | guest at the gate: both screens read 1 / 2 READY and the host cannot enter yet | **PASS** · verified-in-browser | both screens: {"gate":"1 / 2 READY","ready":["alice:false","bob:true"]}; host Enter portal disabled=true |
| 4d | whole crew at the gate: both screens read 2 / 2 READY and the host may enter | **PASS** · verified-in-browser | both screens: {"gate":"2 / 2 READY","ready":["alice:true","bob:true"]}; host Enter portal disabled=false |
| 6f | guest offline -> the host can still start the run | **PASS** · verified-in-browser | crew strip read ["bob Weaver offline","alice Shade not ready"] / gate "READY" with bob gone; host Enter portal disabled=false; phase after the click=expedition |

...
- [ ] **Decide floors on/off for the demo** (§4 + §6). If on: `git merge origin/tuning/demo-preset` (optional but recommended), set the two env vars, restart the server, demo on Vantage Spire or Root Archive.
...
Checkpoint: tag **`overnight-v1`**. Then an agent researched game feel, difficulty curves, build variety, routing, co-op synergy and theme-as-mechanic, wrote `docs/design/V2_PROPOSAL.md` (9 ranked proposals, each tied to a source and to the module it touches, plus 3 suggested cuts) and built the three safest items on branch `next/v2` → **draft PR #46** (not merged; client render/audio/UI only, no sim/protocol change, 1108 tests green):
- a **hit-reaction layer** — 45 ms hit-stop, hit flash, flinch, damage-scaled impact, contact shake (it found `drawImpact` was being passed a constant `1` on every hit);
- **`prefers-reduced-motion` honoured on the canvas** + a menu toggle — the canvas ignores it on main today, which is an accessibility bug you may want to cherry-pick regardless;
- pitch scatter on repeating audio cues.
Proposed but not built, worth a look: a first-biome difficulty ramp with a one-time mercy; treasure rooms offering 1-of-2 boons; elite/treasure/rest revealed ahead on the minimap; and **R7 — each contributor can trigger their own idea once per run as an in-world artefact, and the relic you carry out seeds the next world's prompt** (the proposal argues this is the one mechanic nobody else can copy, because it is the premise made playable).
**Difficulty, resolved with numbers (`docs/TUNING_DEMO_PRESET.md`, `scripts/difficulty-report.ts`):** biome 1 is *not* lethal. The scary bot deaths came from a broken test bot (it backed away to 130 px, further than Bastion or Shade can reach, so melee classes never landed a hit). With a class-aware bot, biome-1 survival with laws on is **99 %** across all 12 class × fixture cells (worst 90 %). The real difficulty problem is the **ending**: a solo Custodian win is only **53 %**. A constants-only **demo preset** lifts that to **78 %** (boss HP 1200→850, hit cap 12→18 %, gentler corrupted floor and hazard ramp, tier scaling 18→10 %/tier, rest heal 40→60 %), Bastion still needs 24–53 s so it isn't trivial. It is on branch **`tuning/demo-preset`** (1080 tests green) — one command to take it: `git merge origin/tuning/demo-preset`. **Crystal Tide stays hard (20–60 %)** because its `glass_lattice` law caps Integrity at 48 — **demo on Vantage Spire or Root Archive.**
